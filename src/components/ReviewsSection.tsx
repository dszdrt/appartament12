  ...
  return (
    <section id="reviews" className="py-24 px-6 relative overflow-hidden bg-charcoal">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <div className="min-h-[320px] sm:min-h-[260px] relative flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="w-full glass-light p-8 md:p-12 rounded-3xl border border-white/10 shadow-2xl relative"
              >
                <Quote className="absolute top-6 right-8 w-14 h-14 text-gold/10 pointer-events-none" />

                {/* Author Info & Source */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="font-serif text-2xl text-warm-white font-bold">
                        {currentReview.authorName}
                      </h3>
                      {currentReview.isPinned && (
                        <span className="text-[10px] uppercase tracking-widest bg-gold/20 text-gold border border-gold/30 px-2 py-0.5 rounded-full font-medium">
                          Рекомендует
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-warm-white/40">{currentReview.dateText}</p>
                  </div>

                  <div className="flex items-center gap-3">
                    <span className="text-xs bg-black/40 text-gold border border-gold/20 px-3 py-1 rounded-full font-medium">
                ...
                {/* Review Text */}
                <p className="text-warm-white/80 text-base md:text-lg leading-relaxed font-light italic">
                  "{currentReview.text}"
                </p>
              </motion.div>
            </AnimatePresence>
          ...
          <div className="flex items-center justify-between mt-8 px-2">
            <div className="flex items-center gap-2">
              <button
                onClick={prevSlide}
                className="p-3 rounded-full bg-white/5 border border-white/10 text-warm-white/70 hover:text-gold hover:border-gold/50 hover:bg-white/10 transition-all duration-300"
              ...
              {reviews.map((_, idx) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    idx === currentIndex ? "w-8 bg-gold" : "w-2 bg-white/20 hover:bg-white/40"
                  }`}
        ...
        <AnimatedSection className="text-center mt-16">
          <a
            ...
            className="btn-gold text-xs py-3.5 px-8 rounded-full inline-flex items-center gap-3 shadow-[0_0_20px_rgba(201,169,110,0.2)]"
          >
